...
fn des_enc(block: &str, round_keys: &[String]) -> String {
    ...
    let combined_text = left.to_owned() + &right;
...
fn main() {
    let my_key = "1010101010111011000010010001100000100111001101101100110011011101";
    let round_keys = generate_keys(my_key);
    let pt_string = "The quick brown fox jumps over the lazy dog";
    println!("Plaintext hex: {}", string_to_hex(pt_string));
    let encrypted_text = des_encrypt_text(pt_string, &round_keys);
    let decrypted_text = des_decrypt_text(&encrypted_text, &round_keys);
    println!("encrypted Hex: {}", binary_to_hex(&encrypted_text));
    println!("encrypted Hex: {}", binary_to_hex(&decrypted_text));
}
