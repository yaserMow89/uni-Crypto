...
fn main() {
    ...
    let mut encrypter = Crypter::new(Cipher::des_ecb(), Mode::Encrypt, key, None).unwrap();

    let block_size = Cipher::des_ecb().block_size();
    ...
    let mut decrypter = Crypter::new(Cipher::des_ecb(), Mode::Decrypt, key, None).unwrap();
