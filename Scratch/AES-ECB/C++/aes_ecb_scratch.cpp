...
// void AES_decrypt(unsigned char* plain_text, const unsigned char* key);
void AES_decrypt(unsigned char * plain_text,  unsigned char * key);
// AES S-box
...
void _print_hex(const std::string& label, const unsigned char* data, int length) {
    // Print label
    std::cout << label << " in Hex:\t";
    // Print data in hexadecimal format
    for (int i = 0; i < length; i++) {
        std::cout << std::hex << std::setfill('0') << std::setw(2) << static_cast<int>(data[i]) << " ";
    }
    std::cout << std::dec << std::endl; // Switch back to decimal for other outputs
}
...
std::vector<unsigned char> key_expansion(const unsigned char* key) {
    std::vector<unsigned char> expanded_key(AES_SIZE * (NUM_ROUNDS + 1));
    memcpy(expanded_key.data(), key, AES_SIZE);

    for (int i = AES_SIZE; i < expanded_key.size(); i += 4) {
        unsigned char temp[4];
        memcpy(temp, expanded_key.data() + i - 4, 4);

        if (i % AES_SIZE == 0) {
            // Rotate the first byte to the end
...
void ShiftRows(unsigned char* state) {
    ...
    // Copy state into tmp with shifts applied
    for (int r = 0; r < 4; ++r) {
        for (int c = 0; c < 4; ++c) {
...
void AES_encrypt(unsigned char * plain_text, unsigned char * key) {
    std::vector<unsigned char> expanded_key = key_expansion(key);
...
// Main function
int main() {
    std::cout <<" ============================== AES ECB (C++) ======================== " <<endl;
    unsigned char plaintext[(AES_SIZE * 2) + 1] = { "Hello World12345" };
    unsigned char ciphertext[(AES_SIZE * 2) + 1]; // To store the ciphertext
    unsigned char key[AES_SIZE + 1] = { "123456789abcdefg" };

    std::cout << "Original Plaintext in Hex:";
    _print_hex("\t", plaintext, 32);

    // Encrypt plaintext and store ciphertext
    memcpy(ciphertext, plaintext, 32); // Copy plaintext to ciphertext buffer
...
aes_ecb_decrypt(ciphertext, key, 32);
    std::cout << "Decrypted Ciphertext in Hex:"; // Print the decrypted ciphertext
    _print_hex("\t", ciphertext, 32);
    return 0;
